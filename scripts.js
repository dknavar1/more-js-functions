...
scoreBasket();
scoreBasket();
addToPage("The score after two baskets and one three point throw: " + scoreThree());
...
function calcChange (amount){
    // calculate the number of quarters, subtract total change
    let quarters = Math.floor(amount / 25);
    amount = amount - (quarters * 25);
    // calculate the number of dimes, subtract total change
    let dimes = Math.floor(amount/ 10);
    amount = amount - (dimes * 10);
    // calculate the number of nickels, subtract total change
